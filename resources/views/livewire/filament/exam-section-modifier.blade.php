<div>
    ...
    <div class="mx-auto bg-gray-100 dark:bg-gray-800 p-5 text-gray-900 dark:text-gray-100">
        <div class="flex justify-between mb-3">
            <button type="button" wire:click="handlePrevious" class="px-4 py-1 flex text-gray-900 dark:text-white rounded-lg hover:text-blue-400 dark:hover:text-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m18.75 4.5-7.5 7.5 7.5 7.5m-6-15L5.25 12l7.5 7.5" />
                </svg>
                Previous
            </button>
            <button type="button" wire:click="handleNext" class="px-4 py-1 flex text-gray-900 dark:text-white rounded-lg hover:text-blue-400 dark:hover:text-blue-400">
                Next
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <input type="text" class="w-full text-2xl font-bold mb-4 text-center border p-2 bg-white dark:bg-gray-700 dark:border-gray-500"
            wire:model="examSections.{{ $page-1 }}.title" placeholder="Enter Title">
        <input type="text" class="w-full text-xl font-bold text-center border p-2 bg-white dark:bg-gray-700 dark:border-gray-500"
            ...
            <div>
                ...
                <h6 class="font-bold grid grid-cols-5 gap-3">
                    ...
                    <input type="text" class="col-span-4 border p-2 bg-white dark:bg-gray-700 dark:border-gray-500"
                        wire:model="examSections.{{ $page-1 }}.problems.{{ $loop->index }}.problem" placeholder="Problem Description">
                </h6>
                <hr class="my-5 border-gray-300 dark:border-gray-500">
                @isset($problem['example'])
                    ...
                    <input type="text" class="mb-2 border p-2 w-full bg-white dark:bg-gray-700 dark:border-gray-500"
                        wire:model="examSections.{{ $page-1 }}.problems.{{ $loop->index }}.example.question" placeholder="Example Question">
                    <ul class="flex gap-8">
                        @foreach ($problem['example']['options'] as $e_option)
                            <li>
                                ...
                                @if ($e_option['is_correct'])
                                    <i class="w-4 h-4 fa-regular fa-circle-check text-green-500"></i>
                                @endif
                            </li>
                        @endforeach
                    </ul>
                ...
                <div class="p-3 rounded-lg mb-6 bg-gray-100 dark:bg-gray-700"
                    ...
                    <div class="space-y-2">
                        @foreach ($question['options'] as $option)
                            <input type="text" class="w-full text-left px-4 py-2 border rounded-md bg-white dark:bg-gray-700 dark:border-gray-500"
    ...
    <button type="button" x-on:click="window.scrollTo({ top: 0, behavior: 'smooth' })"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto w-2/3" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
